 ...
 * 字符串工具类
 * <p/>
 * 项目名称：agr-join-v2.0.0 类名称：StringUtils 类描述： 创建人：lict 创建时间：2015年7月22日 下午12:06:25
 * 修改人：lict 修改时间：2015年7月22日 下午12:06:25 修改备注：
 */
public class StringUtils {
     ...
     * @param srcStr
     * @param size
     * @param ellipsis true 显示省略号
     * @return
     */
    public static String subString(String srcStr, Integer size, boolean ellipsis) {
        if (srcStr == null)
            return "";
        int srcStrLength = srcStr.length();

        if (srcStrLength <= size)
            return srcStr;

        return srcStr.substring(0, size) + (ellipsis ? "..." : "");
    }
     ...
     * 检查字符串是否非空,并添加默认值
     **/
    public static String checkNull(String value, String defalutValue) {
